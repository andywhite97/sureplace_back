...
def authorized(user, r):
    return r.applicant_id == user.id and (
        r.verification_type == VerificationType.IDENTITY
...
@transaction.atomic
def submit(r, user):
    if not authorized(user, r) or r.status != RequestStatus.DRAFT:
        raise ValidationError("Request cannot be submitted.")
    types = set(r.documents.values_list("document_type", flat=True))
    missing = [sorted(x) for x in REQUIRED.get(r.verification_type, []) if not types.intersection(x)]
    if missing:
